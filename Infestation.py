...
def find_closed_restaurant(x, y, fixdist):
    """
    Function that takes 2 float as coordinate and a float as a radius of the
    infected area then return the closed restaurant in Gridbourne based on
    the coordinate
    """
    possiblelist = []
    closedresto = []
    ...
    # Append the restaurants that included in the infected area    
    mindist = fixdist
    for resto in possiblelist:
        if mindist > resto[1]:
            closedresto.append(resto[0])       
    return sorted(closedresto)


def find_restaurants_to_shut(distance, list_of_epicentres):
    """
    Function that takes distance as the radius of the area and 
    lists of coordinates of the epicentres and return the closed restaurant 
    in the radius of all epicentres in the list
    """
    shutlist = []
    
    # Append and return the restaurants that is closed
    for lis in list_of_epicentres:
        closed = find_closed_restaurant(lis[0], lis[1], distance)
        shutlist.append(closed)
    return shutlist